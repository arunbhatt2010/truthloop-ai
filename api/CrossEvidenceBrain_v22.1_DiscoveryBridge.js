...
/*
 * CEB SOURCE FILTRATION
 * ---------------------
 * LinkedIn is deliberately narrowed AFTER both Apify actors have
 * completed and profile enrichment has been merged.
 *
 * Keep:
 *   - full profile/about enrichment
 *   - 1 strongest/latest post
 *   - 1 strongest/latest article
 *
 * The signal engine still sees the selected content in full.
 */
const LINKEDIN_MAX_POSTS = 1;
const LINKEDIN_MAX_ARTICLES = 1;
const SIGNAL_MAX_PER_BUCKET = 8;
...
async function fetchWebsiteEvidence(primaryUrl) {
    ...
    /*
     * PCF may discover social links on pages that are later removed by
     * source filtration. Preserve the aggregated discovery list here so
     * downstream platform routing does not lose LinkedIn/GitHub URLs.
     */
    const discoveredSocialLinks = uniqueUrls([
        ...(Array.isArray(rawPackage?.discoveredSocialLinks)
            ? rawPackage.discoveredSocialLinks
            : []),
        ...(Array.isArray(packageResult?.discoveredSocialLinks)
            ? packageResult.discoveredSocialLinks
            : [])
    ]);
...
function findLinkedInUrl({
    ...
    const discoveredLinkedIn =
        uniqueUrls(discoveredSocialLinks)
            .find(url => detectPlatform(url) === "linkedin");

    if (discoveredLinkedIn) {
        return discoveredLinkedIn;
    }
...
/* ------------------------------------------------------------
   ...
   No ranking. No API call. First supplied/discovered URL is used.
   ------------------------------------------------------------ */
...
function findGitHubUrl({
    ...
    const requestedGitHub =
        uniqueUrls(requestedLinks)
            .find(url => detectPlatform(url) === "github");

    if (requestedGitHub) {
        return requestedGitHub;
    }

    const discoveredGitHub =
        uniqueUrls(discoveredSocialLinks)
            .find(url => detectPlatform(url) === "github");

    if (discoveredGitHub) {
        return discoveredGitHub;
    }
...
/* ------------------------------------------------------------
   GITHUB — DIRECT PUBLIC FETCH
   No Apify actor. Uses the existing PublicContentFetcher path.
   ------------------------------------------------------------ */

async function fetchGitHubEvidence(url = "") {
    ...
    const cached = SOURCE_CACHE.get(normalizedUrl);

    if (cached?.sourcePlatform === "github") {
        return {
            success: true,
            source: cached,
            cached: true
    ...
    try {
        ...
        const sources =
            Array.isArray(packageResult?.sources)
                ? packageResult.sources
                : [];
        ...
        if (!source) {
            return {
                success: false,
                reason: "GitHub public fetch returned no source record.",
                source: null
            };
        }
        ...
        console.log(
            ...
            JSON.stringify({
                ...
                visibleTextLength:
                    safeText(normalizedSource?.visibleText).length
            })
        );
...
/* ------------------------------------------------------------
   LINKEDIN EVIDENCE FILTRATION
   ------------------------------------------------------------ */

function getLinkedInItemText(item = {}) {
    return safeText(
        item?.text ||
        item?.content ||
        item?.description ||
...
function getLinkedInItemDate(item = {}) {
    const raw =
        item?.datePublished ||
        item?.publishedAt ||
        item?.postedAt ||
...
function scoreLinkedInItem(item = {}, type = "post") {
    const text = getLinkedInItemText(item);
    const lower = text.toLowerCase();

    let score = Math.min(text.length, 2500) / 100;

    /*
     * Prefer substantive content over tiny activity records.
     * These are evidence-quality bonuses, not truth claims.
     */
    if (text.length >= 300) score += 8;
    if (text.length >= 700) score += 6;
    if (/[.!?]/.test(text)) score += 2;
    if (/\b(i|we|my|our)\b/i.test(text)) score += 2;
    if (/\b(build|built|launched|created|learned|tested|worked|helped|sold|shipped|failed|discovered)\b/i.test(lower)) {
        score += 5;
    }
    if (/\b(client|customer|user|founder|startup|product|revenue|sales|problem|lesson|experiment|result)\b/i.test(lower)) {
        score += 4;
    }

    if (type === "article") score += 1;

    return score;
}

function filterLinkedInEvidence(source = null) {
    ...
    const originalPosts = Array.isArray(source?.posts)
        ? source.posts
        : [];

    const originalArticles = Array.isArray(source?.articles)
        ? source.articles
        : [];

    const selectOne = (items, type) => {
        return [...items]
            .filter(item => item && typeof item === "object")
            .sort((a, b) => {
                const dateDiff =
                    getLinkedInItemDate(b) -
                    getLinkedInItemDate(a);

                if (dateDiff !== 0) return dateDiff;

                return (
                    scoreLinkedInItem(b, type) -
                    scoreLinkedInItem(a, type)
                );
            })
            .slice(0, 1);
    };
    ...
    const selectedPosts =
        selectOne(originalPosts, "post")
            .slice(0, LINKEDIN_MAX_POSTS);

    const selectedArticles =
        selectOne(originalArticles, "article")
            .slice(0, LINKEDIN_MAX_ARTICLES);
    ...
    const filteredSource = {
        ...
        /*
         * Rebuild visibleText from retained LinkedIn evidence so the
         * downstream signal engine cannot accidentally consume dropped
         * activity through the legacy aggregate field.
         */
        visibleText: [
            source?.linkedinProfile?.name
                ? `Name: ${safeText(source.linkedinProfile.name)}`
                : "",
            source?.linkedinProfile?.headline
                ? `Headline: ${safeText(source.linkedinProfile.headline)}`
                : "",
            source?.linkedinProfile?.about
                ? `About: ${safeText(source.linkedinProfile.about)}`
                : "",
            ...selectedPosts.map(getLinkedInItemText),
            ...selectedArticles.map(getLinkedInItemText)
        ]
            .filter(Boolean)
            .join("\n"),
    ...
    return {
        ...
        keptPosts: selectedPosts.length,
        keptArticles: selectedArticles.length
    };
}

function uniqueSignalObjects(values = []) {
    const seen = new Set();
    const result = [];
    ...
    for (const item of Array.isArray(values) ? values : []) {
        ...
        const key = [
            safeText(item?.signal).toLowerCase(),
            safeText(item?.basis).toLowerCase()
        ].join("|");
...
function capSignalBucket(values = [], max = SIGNAL_MAX_PER_BUCKET) {
    return uniqueSignalObjects(values).slice(0, max);
}
...
async function buildSignalSignals({
    ...
    /*
     * HIGH-DENSITY SIGNAL EXTRACTION
     * -------------------------------
     * Extract many small, evidence-grounded signals instead of a few
     * generic topic labels. No AI inference and no invented facts.
     */
    const addUniqueSignal = (
        ...
        addSignal(bucket, text, urls, basis);
    };
    ...
    const sentencePool = [];
    for (const text of signalTextParts) {
    ...
    const uniqueSentences = [
        ...new Map(
            sentencePool.map(sentence => [
                normalizeTokenText(sentence),
                sentence
            ])
        ).values()
    ];

    const firstPersonSentences =
        uniqueSentences.filter(sentence =>
            /\b(i|we|my|our|me|us)\b/i.test(sentence)
        );

    for (const sentence of firstPersonSentences.slice(0, 8)) {
        addUniqueSignal(
            contentPatternSignals,
            `First-person public writing contains: "${sentence}"`,
            supportUrls,
            "Direct sentence-level evidence from supplied public content."
        );
    }

    const actionFamilies = {
        building: /\b(build|built|building|create|created|creating|launch|launched|ship|shipped|develop|developed)\b/i,
        learning: /\b(learn|learned|learning|lesson|discovered|realized|figured out)\b/i,
        experimentation: /\b(test|tested|testing|experiment|experiments|trial|validated|validation)\b/i,
        execution: /\b(action|execute|execution|implement|implemented|doing|done|ship|shipped)\b/i,
        commercial: /\b(sell|sold|sales|client|clients|customer|customers|revenue|pricing|paid|payment)\b/i,
        teaching: /\b(teach|teaching|coach|coaching|advice|advise|guide|how to|learn)\b/i,
        failure: /\b(fail|failed|failure|mistake|wrong|struggle|stuck|problem)\b/i
    };

    for (const [family, pattern] of Object.entries(actionFamilies)) {
        const matches = uniqueSentences.filter(sentence =>
        ...
        if (matches.length > 0) {
            addUniqueSignal(
                contentPatternSignals,
                `Public writing repeatedly contains ${family}-oriented language.`,
                supportUrls,
                `Observed ${matches.length} sentence-level matches in supplied evidence.`
            );

            for (const sentence of matches.slice(0, 2)) {
                addUniqueSignal(
                    proofSignals,
                    `Evidence example for ${family}: "${sentence}"`,
                    supportUrls,
                    "Direct sentence-level evidence."
                );
            }
        }
    }

    const quantifiedSentences =
        uniqueSentences.filter(sentence =>
            /\b\d+(?:\.\d+)?\s*(?:%|percent|users?|clients?|customers?|years?|months?|days?|posts?|followers?|people|x)\b/i
                .test(sentence)
        );

    for (const sentence of quantifiedSentences.slice(0, 6)) {
        addUniqueSignal(
            proofSignals,
            `Public content contains a quantified claim: "${sentence}"`,
            supportUrls,
            "Directly detected numeric quantity in public text; claim itself is not independently verified."
        );
    }

    const questionSentences =
        uniqueSentences.filter(sentence => sentence.includes("?"));

    for (const sentence of questionSentences.slice(0, 5)) {
        addUniqueSignal(
            contentPatternSignals,
            `Public writing uses question framing: "${sentence}"`,
            supportUrls,
            "Direct question sentence from supplied evidence."
        );
    }

    const repeatedPhraseMap = new Map();
    for (const sentence of uniqueSentences) {
        const words = normalizeTokenText(sentence)
            .split(/\s+/)
            .filter(word =>
                word.length >= 5 &&
                !/^(there|their|about|which|would|could|should|these|those|where|while|because|through)$/.test(word)
            );
        ...
        for (const word of words) {
            ...
            repeatedPhraseMap.set(
                word,
                (repeatedPhraseMap.get(word) || 0) + 1
            );
        }
    }

    const recurringLexicalThemes =
        [...repeatedPhraseMap.entries()]
            .filter(([, count]) => count >= 3)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 10);

    for (const [term, count] of recurringLexicalThemes) {
        addUniqueSignal(
            contentPatternSignals,
            `The term "${term}" recurs across public writing.`,
            supportUrls,
            `Observed ${count} sentence-level occurrences after normalization.`
        );
    }

    const crossSourcePhraseCandidates =
        [...new Set(
            uniqueSentences
                .map(sentence =>
                    normalizeTokenText(sentence)
    ...
    for (const term of crossSourcePhraseCandidates.slice(0, 80)) {
        const websiteHits =
            countMatches(websiteText, [term]);
        const linkedinHits =
            countMatches(linkedinText, [term]);

        if (websiteHits > 0 && linkedinHits > 0) {
            addUniqueSignal(
                crossSourceSignals,
                `The term "${term}" appears in both website and LinkedIn evidence.`,
                [...websiteUrls, ...linkedinUrls],
                `Observed ${websiteHits} website and ${linkedinHits} LinkedIn occurrence(s).`
            );
        }
    }
    ...
    if (linkedinAbout) {
        addUniqueSignal(
            positioning,
            `LinkedIn About provides explicit self-positioning language.`,
            [linkedin?.linkedinProfile?.profileUrl || linkedin?.sourceUrl],
            `About section contains ${linkedinAbout.length} characters of supplied text.`
        );

        for (const sentence of linkedinAbout
            .split(/(?<=[.!?])\s+/)
            .map(clean)
            ...
            addUniqueSignal(
                proofSignals,
                `LinkedIn About evidence: "${sentence}"`,
                [linkedin?.linkedinProfile?.profileUrl || linkedin?.sourceUrl],
                "Direct sentence from the LinkedIn About field."
            );
        }
    }

    const selectedPostText =
        linkedinPosts.length
            ? getLinkedInItemText(linkedinPosts[0])
            : "";

    const selectedArticleText =
        linkedinArticles.length
            ? getLinkedInItemText(linkedinArticles[0])
            : "";

    if (selectedPostText) {
        addUniqueSignal(
            creatorSignals,
            "LinkedIn has one retained substantive activity item for downstream investigation.",
            linkedinUrls,
            `Selected post contains ${selectedPostText.length} characters.`
        );
    }
    ...
    if (selectedArticleText) {
        addUniqueSignal(
            creatorSignals,
            "LinkedIn has one retained article item for downstream investigation.",
            linkedinUrls,
            `Selected article contains ${selectedArticleText.length} characters.`
        );
    }

    const contradictionPatterns = [
        {
            left: /\b(no advice|not advice|no coaching|not coaching)\b/i,
            right: /\b(advice|coaching|coach|guide|how to|teach)\b/i,
            text: "Website positioning contains no-advice/no-coaching language while LinkedIn contains advisory/teaching language."
        },
        {
            left: /\b(building|building in public|build)\b/i,
            right: /\b(no longer building|stopped building|not building)\b/i,
            text: "Different supplied sources contain potentially conflicting building-state language."
        }
    ];

    for (const item of contradictionPatterns) {
        if (item.left.test(websiteText) && item.right.test(linkedinText)) {
            addUniqueSignal(
                contradictions,
                item.text,
                [...websiteUrls, ...linkedinUrls],
                "Both textual signals were directly detected in separate supplied source groups."
            );
        }
    }

    /*
     * Keep signal density high while preventing pathological repetition.
     */
    positioning.splice(0, positioning.length, ...capSignalBucket(positioning));
    expertiseSignals.splice(0, expertiseSignals.length, ...capSignalBucket(expertiseSignals));
    audienceSignals.splice(0, audienceSignals.length, ...capSignalBucket(audienceSignals));
    businessSignals.splice(0, businessSignals.length, ...capSignalBucket(businessSignals));
    creatorSignals.splice(0, creatorSignals.length, ...capSignalBucket(creatorSignals));
    contentPatternSignals.splice(0, contentPatternSignals.length, ...capSignalBucket(contentPatternSignals));
    proofSignals.splice(0, proofSignals.length, ...capSignalBucket(proofSignals));
    behavioralSignals.splice(0, behavioralSignals.length, ...capSignalBucket(behavioralSignals));
    contradictions.splice(0, contradictions.length, ...capSignalBucket(contradictions));
    crossSourceSignals.splice(0, crossSourceSignals.length, ...capSignalBucket(crossSourceSignals));
...
function buildUniversalPackage({
    ...
    const compressedGithubSource =
        githubSource
            ? compressUniversalSource(githubSource)
            : null;
...
console.log(
    ...
    if (detectPlatform(primaryUrl) === "linkedin") {
    ...
    } else if (detectPlatform(primaryUrl) === "github") {

        /*
         * Primary source itself is GitHub.
         * Fetch it directly; no Apify actor is involved.
         */
        ...
        if (suppliedWebsiteSources.length > 0) {
            ...
            discoveredSocialLinks = uniqueUrls([
                ...(Array.isArray(footprintPackage?.discoveredSocialLinks)
                    ? footprintPackage.discoveredSocialLinks
                    : []),
                ...(Array.isArray(footprintPackage?.websiteEvidence?.discoveredSocialLinks)
                    ? footprintPackage.websiteEvidence.discoveredSocialLinks
                    : [])
            ]);
            ...
            if (websiteResult?.success) {
                ...
                discoveredSocialLinks =
                    Array.isArray(websiteResult?.discoveredSocialLinks)
                        ? websiteResult.discoveredSocialLinks
                        : [];
            }
        }
    ...
    if (githubUrl) {
        const githubResult =
            await fetchGitHubEvidence(githubUrl);
     ...
     * LINKEDIN FILTER
     * ---------------
     * Both actors have completed and profile enrichment has been merged.
     * Now retain only the evidence that is most useful downstream.
     */
    const linkedinFilterResult =
        filterLinkedInEvidence(linkedinSource);

    linkedinSource =
        linkedinFilterResult.source;

    console.log(
        "CEB_LINKEDIN_FILTRATION",
        JSON.stringify({
            originalPosts:
                linkedinFilterResult.originalPosts,
            originalArticles:
                linkedinFilterResult.originalArticles,
            keptPosts:
                linkedinFilterResult.keptPosts,
            keptArticles:
                linkedinFilterResult.keptArticles
        })
    );

    /*
     * Signal gets the FILTERED LinkedIn evidence plus the already-filtered
     * website evidence. This maximizes signal density without carrying
     * eight or ten redundant LinkedIn items into ECB.
     */
    const signalResult =
