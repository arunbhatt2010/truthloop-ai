...
const MAX_GEMINI_CONTENT_ITEMS = 10;
...
Your ONLY job is to derive high-level intelligence signals from the
already collected public evidence supplied below.

You are NOT an evidence collector.
You are NOT an evidence compressor.
You are NOT an evidence filter.
You are NOT an article selector.
You are NOT a post selector.
You are NOT a source selector.
You are NOT a report writer.
You are NOT a psychological diagnostician.
You do NOT fetch or invent anything.

The upstream systems have already collected and preserved the public
sources. Do not delete, rank, summarize, rewrite, merge, or replace
those sources. Raw evidence is owned by CrossEvidenceBrain and remains
outside your output.

Your task is ONLY to infer compact, evidence-grounded SIGNALS that
Loop 7 can use alongside the preserved Universal Public Evidence Package.

SIGNAL RULES:
- Use only the supplied evidence.
- Do not invent facts.
- Do not invent URLs, names, companies, audiences, expertise, activity,
  behavior, or outcomes.
- Do not diagnose psychology or mental health.
- Do not provide advice.
- Do not create an alternative evidence package.
- Do not output copied article/post bodies.
- Do not output sourceContent.
- Do not output contentSamples.
- Do not output selected/filtered articles or posts.
- Do not decide which public source is important enough to survive.
- Do not remove or rank evidence; the upstream package already preserves it.
- A signal must be supported by one or more supplied source URLs.
- Prefer repeated cross-source signals over one-off claims.
- When evidence is insufficient, return [] or null.

IMPORTANT:
Every signal must carry its supporting source URLs when possible.
These URLs are references only; the original evidence remains in the
...
Where a signal is substantive, prefer:
{
  "signal": "...",
...
For topics or simple labels, plain strings are acceptable.
Do not exceed compact output. Do not return raw evidence.
...
`;
    ...
    try {
        console.log(
            "GEMINI_SIGNAL_INPUT",
            JSON.stringify({
                sourceCount: evidencePackage?.sourceCount || 0,
                selectedSourceCount:
                    Array.isArray(evidencePackage?.sources)
                        ? evidencePackage.sources.length
                        : 0,
                contentSourceCount:
                    Array.isArray(evidencePackage?.contentSources)
                        ? evidencePackage.contentSources.length
                        : 0,
                socialSourceCount:
                    Array.isArray(evidencePackage?.socialSources)
                        ? evidencePackage.socialSources.length
                        : 0
            })
        );
        ...
        if (!content.trim()) {
            return {
                status: "failed",
                error: "Empty Gemini signal response"
            };
        }
        ...
        if (!parsedContent || typeof parsedContent !== "object") {
            return {
                status: "failed",
                error: "Gemini returned invalid signal object"
            };
        }

        // Gemini is a signal generator only. Raw public evidence remains
        // owned and preserved by CrossEvidenceBrain outside this object.
        const signalPackage = {
            status: "ok",
            identity: parsedContent?.identity || {},
            positioning: Array.isArray(parsedContent?.positioning)
                ? parsedContent.positioning.slice(0, 12)
                : [],
            niches: Array.isArray(parsedContent?.niches)
                ? parsedContent.niches.slice(0, 12)
                : [],
            expertiseSignals: Array.isArray(parsedContent?.expertiseSignals)
                ? parsedContent.expertiseSignals.slice(0, 12)
                : [],
            audienceSignals: Array.isArray(parsedContent?.audienceSignals)
                ? parsedContent.audienceSignals.slice(0, 12)
                : [],
            businessSignals: Array.isArray(parsedContent?.businessSignals)
                ? parsedContent.businessSignals.slice(0, 12)
                : [],
            creatorSignals: Array.isArray(parsedContent?.creatorSignals)
                ? parsedContent.creatorSignals.slice(0, 12)
                : [],
            topics: Array.isArray(parsedContent?.topics)
                ? parsedContent.topics.slice(0, 12)
                : [],
            recurringTopics: Array.isArray(parsedContent?.recurringTopics)
                ? parsedContent.recurringTopics.slice(0, 12)
                : [],
            behavioralSignals: Array.isArray(parsedContent?.behavioralSignals)
                ? parsedContent.behavioralSignals.slice(0, 12)
                : [],
            contradictions: Array.isArray(parsedContent?.contradictions)
                ? parsedContent.contradictions.slice(0, 12)
                : [],
            crossSourceSignals: Array.isArray(parsedContent?.crossSourceSignals)
                ? parsedContent.crossSourceSignals.slice(0, 12)
                : [],
            signalConfidence:
                Number.isFinite(Number(parsedContent?.signalConfidence))
                    ? Math.max(0, Math.min(100, Number(parsedContent.signalConfidence)))
                    : 0
        };

        console.log(
            "GEMINI_SIGNAL_OUTPUT",
            JSON.stringify({
                status: signalPackage.status,
                positioning: signalPackage.positioning.length,
                expertiseSignals: signalPackage.expertiseSignals.length,
                audienceSignals: signalPackage.audienceSignals.length,
                businessSignals: signalPackage.businessSignals.length,
                creatorSignals: signalPackage.creatorSignals.length,
                topics: signalPackage.topics.length,
                recurringTopics: signalPackage.recurringTopics.length,
                behavioralSignals: signalPackage.behavioralSignals.length,
                contradictions: signalPackage.contradictions.length,
                crossSourceSignals: signalPackage.crossSourceSignals.length,
                signalConfidence: signalPackage.signalConfidence
            })
        );
        ...
        return {
            status: "failed",
            error: error?.message || "Gemini signal generation failed."
        };
    }
}
...
function normalizeGeminiIntelligence(intelligence = {}, selection = {}) {
    ...
    return {
        status: intelligence.status || "ok",
        identity: intelligence.identity || {},
        positioning: Array.isArray(intelligence.positioning)
            ? intelligence.positioning
            : [],
        niches: Array.isArray(intelligence.niches)
            ? intelligence.niches
            : [],
        expertiseSignals: Array.isArray(intelligence.expertiseSignals)
            ? intelligence.expertiseSignals
            : [],
        audienceSignals: Array.isArray(intelligence.audienceSignals)
            ? intelligence.audienceSignals
            : [],
        businessSignals: Array.isArray(intelligence.businessSignals)
            ? intelligence.businessSignals
            : [],
        creatorSignals: Array.isArray(intelligence.creatorSignals)
            ? intelligence.creatorSignals
            : [],
        topics: Array.isArray(intelligence.topics)
            ? intelligence.topics
            : [],
        recurringTopics: Array.isArray(intelligence.recurringTopics)
            ? intelligence.recurringTopics
            : [],
        behavioralSignals: Array.isArray(intelligence.behavioralSignals)
            ? intelligence.behavioralSignals
            : [],
        contradictions: Array.isArray(intelligence.contradictions)
            ? intelligence.contradictions
            : [],
        crossSourceSignals: Array.isArray(intelligence.crossSourceSignals)
            ? intelligence.crossSourceSignals
            : [],
        signalConfidence:
            Number.isFinite(Number(intelligence.signalConfidence))
                ? Number(intelligence.signalConfidence)
                : 0,
        investigation: {
  ...
  JSON.stringify(
    ...
    console.log(
        ...
        JSON.stringify({
            ...
            positioning: geminiIntelligence?.positioning?.length || 0,
            expertiseSignals: geminiIntelligence?.expertiseSignals?.length || 0,
            audienceSignals: geminiIntelligence?.audienceSignals?.length || 0,
            businessSignals: geminiIntelligence?.businessSignals?.length || 0,
            creatorSignals: geminiIntelligence?.creatorSignals?.length || 0,
            topics: geminiIntelligence?.topics?.length || 0,
            recurringTopics: geminiIntelligence?.recurringTopics?.length || 0,
            behavioralSignals: geminiIntelligence?.behavioralSignals?.length || 0,
            contradictions: geminiIntelligence?.contradictions?.length || 0,
            crossSourceSignals: geminiIntelligence?.crossSourceSignals?.length || 0
        })
    );
