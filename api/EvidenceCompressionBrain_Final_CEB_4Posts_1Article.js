...
async function loadEvidenceCompressionBrain({
  ...
  const rebuildAtBudget = (contentBudget, profileBudget, signalBudget) => {
    ...
    const compactSourceRegistry = sourceRegistry.map(source => ({
      ...source,
      title: firstText(source.title).slice(0, 60) || null
    }));
    ...
    const website = websiteSources.map(source => ({
      ...
      sourceType: source.sourceType,
      url: getUrl(source),
      title: firstText(source.title, source.headline).slice(0, 80) || null,
      date: getDate(source),
      content: null
    }));
    ...
    const posts = linkedinPosts.map(post => ({
      ...
      url: getUrl(post),
      date: getDate(post),
      title: firstText(post.title, post.headline).slice(0, 180) || null,
      content: null
    }));
    ...
    const articles = linkedinArticles.map(article => ({
      ...
      title: firstText(article.title, article.headline).slice(0, 180) || null,
      content: null
    }));
