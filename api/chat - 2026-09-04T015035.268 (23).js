...
The report you receive is the raw investigation report generated by Evidence Compression Brain (ECB).

ROLE
Act only as the final editorial layer.
Do not investigate again.
Do not re-analyze evidence.
Do not create new findings, patterns, mechanisms, contradictions, confidence judgments, or facts.
Do not change the priority or meaning of the ECB investigation.

TASK
Apply the final editorial pass to the completed ECB report.
Improve grammar, clarity, flow, readability, and presentation.
Repair incomplete sentences or formatting only when necessary.
Remove repetition only when the underlying finding and evidence remain unchanged.

SOURCE TRACEABILITY
Preserve every source citation exactly as supplied.
Use only source IDs present in sourceRegistry.
Never invent, renumber, rename, or convert source IDs.
...
LENGTH
Keep the final report within 5000 characters.
Complete all 8 sections.
Never truncate a sentence or section.
Do not expand the report beyond the supplied ECB investigation.

INPUT
1. investigationReport = completed raw ECB investigation.
2. sourceRegistry = authoritative source registry.

Return ONLY the finished final report.
`;
...
if (loopLevel === 7) {
  ...
  const normalizeCitationKey = id => {
    const match = String(id || "").trim().match(/^(?:SOURCE|EVIDENCE)_(\d{1,3})$/i);
    return match ? String(Number(match[1])) : String(id || "").toUpperCase();
  };

  const registryKeys = new Set(
    [...registryIds].map(normalizeCitationKey)
  );

  const citedIds = [
    ...String(value || "").matchAll(/\[((?:SOURCE|EVIDENCE)_\d{1,3})\]/gi)
  ].map(match => match[1].toUpperCase());

  const invalidCitations = citedIds.filter(
    id => !registryKeys.has(normalizeCitationKey(id))
  );
  ...
  const evidenceBody =
    `${byHeading["📋 Investigation Summary"] || ""} ` +
    `${byHeading["🧩 Behavioral Findings"] || ""} ` +
    `${byHeading["🌐 Public Evidence"] || ""} ` +
    `${byHeading["🔍 Cross Evidence"] || ""}`;

  if (invalidCitations.length) return false;

  if (
    publicEvidenceExists &&
    uniqueValidCitations.size < Math.min(4, registryIds.size)
  ) {
    return false;
  }
  ...
  if (
    publicEvidenceExists &&
    /zero public evidence|no public evidence|absence of public evidence/i.test(
      evidenceBody
    )
  ) {
    return false;
  ...
  return (
    ...
    hasTerms(
      "⚙ Hidden Mechanism",
      ["trigger", "reinforcement", "conclusion"],
      100
    ) &&
    hasTerms(
      "🌐 Public Evidence",
      ["source", "observation", "evidence summary"],
      publicEvidenceExists ? 160 : 100
    ) &&
    hasTerms(
      "🔍 Cross Evidence",
      ["corroboration", "contradiction", "consistency assessment"],
      120
    ) &&
    hasTerms(
      "📊 Evidence Confidence",
