...
CASE HIERARCHY
1. Loop 1–6 profile cards + compact user case timeline = PRIMARY CASE SPINE.
2. Public evidence = SECONDARY evidence.
3. Use public evidence to corroborate, challenge, contextualize, or expose evidence gaps.
4. Never let a public topic alone prove a private motive, belief, or behavior.
...
- 🌐 Public Evidence: Do not stop at what a source says. Analyze the most decision-relevant observable behavior, including timing/recency when dates exist. Surface meaningful changes, gaps, repeated output, or what has stopped happening when actually supported.
...
- 📊 Evidence Confidence: Grade claims by evidentiary strength. Separate direct evidence, corroborating evidence, and inference. Do not call something high confidence merely because multiple sources repeat the same theme.
- 💡 Final Reflection: Synthesize the strongest supported pattern and its sustaining mechanism. Move beyond repeating earlier conclusions and identify the most consequential unresolved tension or breakpoint.
...
if (loopLevel === 7) {
  ...
  const reportIsComplete = value => {
    ...
    return sections.length === requiredLoop7Sections.length &&
      sections.every(({ body }) =>
        body.length >= 90 && !emptyOnly.has(body.toLowerCase())
      );
  };
  ...
  const reportPassesInvestigationGate = value => {
    ...
    const mustContain = (heading, terms, minLength) => {
    ...
    return (
      mustContain("📋 Investigation Summary", ["finding", "evidence", "conclusion"], 260) &&
      mustContain("🧩 Behavioral Findings", ["pattern", "evidence", "conclusion"], 240) &&
      mustContain("⚙ Hidden Mechanism", ["trigger", "reinforcement", "conclusion"], 220) &&
      mustContain("🌐 Public Evidence", ["source", "observation", "evidence summary"], 260) &&
      mustContain("🔍 Cross Evidence", ["corroboration", "contradiction", "consistency assessment"], 240) &&
      mustContain("📊 Evidence Confidence", ["strong evidence", "weak evidence", "confidence assessment"], 220) &&
      mustContain("💡 Final Reflection", ["observation", "final conclusion"], 220) &&
      mustContain("🎯 One Next Action", ["recommended action"], 90)
    );
  };
  ...
  if (!reportPassesInvestigationGate(reply)) {
    console.log("LOOP7_REPORT_RETRY", "completeness_or_investigation_gate_failed");
    ...
    try {
            ...
            messages: [
              {
                ...
                content:
                  loop7Instruction +
                  "\n\nEMERGENCY COMPLETION REQUIREMENT:\n" +
                  "The previous draft failed the strict completeness/investigation gate. Rewrite the report from the evidenceRegistry. " +
                  "Every section must contain substantive investigative reasoning, not summary, praise, or repetition. " +
                  "Actively inspect dates, recency, publishing gaps, behavioral changes, contradictions, and evidence gaps when supported. " +
                  "Do not hide a material gap with confident language. Finish all eight sections completely. " +
                  "Do not omit or truncate Final Reflection or One Next Action. " +
                  "Use the full available output budget."
              },
              {
                role: "user",
      ...
      if (retryResponse.ok) {
        ...
        if (reportPassesInvestigationGate(retryReply)) {
          reply = normalizeLoop7SectionHeadings(
            normalizeLoop7BulletLayout(retryReply)
          );

          console.log(
            "LOOP7_REPORT_RETRY_SUCCESS",
            JSON.stringify({ chars: reply.length })
          );
        } else {
          console.warn(
            "LOOP7_REPORT_RETRY_STILL_INCOMPLETE",
            JSON.stringify({ chars: retryReply.length })
          );
        }
      } else {
