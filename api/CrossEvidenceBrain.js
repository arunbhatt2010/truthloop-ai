...
function extractTopics(source = {}) {
    ...
    return dictionary.filter(term => text.includes(term)).slice(0, 8);
}
...
function trimPackageToBudget(pkg, maxChars = 60000) {
    ...
    const trimmed = {
        ...
        sources: (pkg?.sources || []).map(source => ({
            ...
            visibleText: cleanText(source?.visibleText || source?.contentSnippet || "", 1800),
            contentSnippet: cleanText(source?.contentSnippet || source?.visibleText || "", 900),
            contentLength: source?.contentLength || 0,
            topics: Array.isArray(source?.topics) ? source.topics.slice(0, 8) : [],
            socialLinks: Array.isArray(source?.socialLinks) ? source.socialLinks.slice(0, 12) : [],
            socialProfiles: Array.isArray(source?.socialProfiles) ? source.socialProfiles.slice(0, 12) : [],
            links: Array.isArray(source?.links) ? source.links.slice(0, 20) : [],
            evidence: Array.isArray(source?.evidence) ? source.evidence.slice(0, 4) : []
        })),
        findings: Array.isArray(pkg?.findings) ? pkg.findings.slice(0, 10) : [],
        evidenceLedger: Array.isArray(pkg?.evidenceLedger) ? pkg.evidenceLedger.slice(0, 80) : []
    };
    ...
    return {
        ...
        version: pkg?.version || "21.1",
        mainProfileLink: pkg?.mainProfileLink || null,
        sourceLinks: Array.isArray(pkg?.sourceLinks) ? pkg.sourceLinks.slice(0, 20) : [],
        discoveredProfiles: Array.isArray(pkg?.discoveredProfiles) ? pkg.discoveredProfiles.slice(0, 20) : [],
        platforms: Array.isArray(pkg?.platforms) ? pkg.platforms.slice(0, 20) : [],
        confidence: pkg?.confidence || 0,
        investigationQueue: Array.isArray(pkg?.investigationQueue) ? pkg.investigationQueue.slice(0, 21) : [],
        findings: Array.isArray(pkg?.findings) ? pkg.findings.slice(0, 8) : [],
        repeatedTopics: Array.isArray(pkg?.repeatedTopics) ? pkg.repeatedTopics.slice(0, 10) : [],
        evidenceLedger: Array.isArray(pkg?.evidenceLedger) ? pkg.evidenceLedger.slice(0, 60) : [],
        sources: (Array.isArray(pkg?.sources) ? pkg.sources : []).slice(0, 21).map(source => ({
            sourceUrl: source?.sourceUrl || null,
            sourcePlatform: source?.sourcePlatform || null,
            title: source?.title || null,
            description: source?.description || null,
            contentSnippet: cleanText(source?.contentSnippet || source?.visibleText || "", 500),
            contentLength: source?.contentLength || 0,
            topics: Array.isArray(source?.topics) ? source.topics.slice(0, 8) : [],
            socialLinks: Array.isArray(source?.socialLinks) ? source.socialLinks.slice(0, 8) : [],
...
export async function loadCrossEvidenceBrain({
    ...
    const universalPackage = {
        ...
        repeatedTopics: cross.repeatedTopics,
        evidenceLedger,
        // Canonical legacy field retained for ECB/chat.js compatibility.
        intelligence: geminiIntelligence,
        // New descriptive alias retained without breaking old consumers.
    ...
    result.crossEvidencePackage = {
        ...
        confidence,
        evidenceLedger,
        intelligence: universalPackage.intelligence,
        investigation: universalPackage.investigation,
        universalPackage
    };
