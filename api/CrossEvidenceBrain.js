...
const APIFY_ACTOR_ID = "bovi~linkedin-profile-scraper";
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------
   Runs AFTER PCF + Apify + Gemini collection and BEFORE export.
...
const UNIVERSAL_CONTENT_MAX_CHARS = 1600;
const UNIVERSAL_ITEM_TEXT_MAX_CHARS = 700;
const UNIVERSAL_ABOUT_MAX_CHARS = 1800;
const UNIVERSAL_GEMINI_TEXT_MAX_CHARS = 700;
    ...
    if (value && typeof value === "object") {
        ...
        for (const [childKey, childValue] of Object.entries(value)) {
            ...
            if (
                ...
                childMax = UNIVERSAL_GEMINI_TEXT_MAX_CHARS;
            }
...
function compressUniversalSignals(signals = {}) {
    return compressUniversalContent(
        signals,
        UNIVERSAL_GEMINI_TEXT_MAX_CHARS
    );
}
        ...
        const posts =
            Array.isArray(rawProfile?.posts)
                ...
                : Array.isArray(rawProfile?.recentPosts)
                    ...
                    : Array.isArray(rawProfile?.recent_posts)
                        ? rawProfile.recent_posts
                        : Array.isArray(item?.posts)
                            ? item.posts
                            : Array.isArray(item?.recent_posts)
                                ? item.recent_posts
                                : [];
        ...
        const articles =
            Array.isArray(rawProfile?.articles)
                ...
                : Array.isArray(rawProfile?.recentArticles)
                    ...
                    : Array.isArray(rawProfile?.recent_articles)
                        ? rawProfile.recent_articles
                        : Array.isArray(item?.articles)
                            ? item.articles
                            : Array.isArray(item?.recent_articles)
        ...
        console.log(
            ...
            JSON.stringify({
                ...
                hasExperience:
                    ...
                    Array.isArray(rawProfile?.experienceHistory)
            })
        );
...
function buildUniversalPackage({
     ...
     * Gemini signal extraction has already completed.
        ...
        JSON.stringify({
            ...
            linkedinArticles:
                Array.isArray(compressedLinkedinSource?.articles)
                    ...
                    : 0,
            geminiSignals:
                Object.keys(compressedGeminiSignals || {}).length
        })
    );
