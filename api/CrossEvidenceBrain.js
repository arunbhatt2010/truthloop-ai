...
// Evidence packaging budget for the downstream Loop 7 investigator.
...
const MAX_INVESTIGATION_SOURCE_CHARS = 3500;
const MAX_INVESTIGATION_EVIDENCE_SNIPPETS = 8;
const MAX_INVESTIGATION_CONTENT_ITEMS = 6;
const MAX_INVESTIGATION_CONTENT_ITEM_CHARS = 700;
...
function buildInvestigationContentItems(source = {}, sourceUrl = "", sourceRole = "") {
    const items = [];

    const addItem = (type, title, content, url = sourceUrl) => {
        const text = cleanText(content || "", MAX_INVESTIGATION_CONTENT_ITEM_CHARS);
    ...
    if (Array.isArray(source.posts)) {
        source.posts.slice(0, MAX_INVESTIGATION_CONTENT_ITEMS).forEach((post, index) => {
    ...
    if (Array.isArray(source.articles)) {
        source.articles.slice(0, MAX_INVESTIGATION_CONTENT_ITEMS).forEach((article, index) => {
    ...
    return items.slice(0, MAX_INVESTIGATION_CONTENT_ITEMS);
}

function buildInvestigationEvidencePackage(sources = [], selection = {}) {
    ...
    source.evidence = [
        ...
        selectedSources.push({
            ...
            visibleText: cleanText(
                ...
                MAX_INVESTIGATION_SOURCE_CHARS
            ) || null,
            ...
            articles: Array.isArray(source?.articles)
                ? source.articles.slice(0, MAX_INVESTIGATION_CONTENT_ITEMS)
                : [],
            posts: Array.isArray(source?.posts)
                ? source.posts.slice(0, MAX_INVESTIGATION_CONTENT_ITEMS)
                : [],
            contentSamples: buildInvestigationContentItems(
                source,
                sourceUrl,
                sourceRole
            ),
            ...
            evidence: Array.isArray(source?.evidence)
                ? source.evidence.slice(0, MAX_INVESTIGATION_EVIDENCE_SNIPPETS)
                : []
        });
    }
...
console.log(
  ...
  Date.now()
        ...
        const source = {
            ...
            evidence: evidence.slice(0, MAX_INVESTIGATION_EVIDENCE_SNIPPETS)
        };
...
export async function loadCrossEvidenceBrain({
    ...
    // SOCIAL MEDIA CONTENT FETCHER: fetch only the social/profile URLs
    // already discovered by the primary public source.
    const socialMediaContentFetcher = await SocialMediaContentFetcher({
...
console.log(
  ...
  JSON.stringify(
    ...
    // CrossEvidenceBrain is evidence-only. No LLM call is made here.
    // Final reasoning is delegated to the downstream Loop 7 investigator.
    const investigationInput = buildInvestigationEvidencePackage(
        sources,
        {
            ...selection,
    ...
    console.log(
        "INVESTIGATION_EVIDENCE_AUDIT",
        JSON.stringify(
            (investigationInput.sources || []).map(source => ({
    ...
    console.log(
        "INVESTIGATION_TARGETED_SOURCES",
        JSON.stringify({
            mainProfile: investigationInput.mainProfile,
            contentSources: investigationInput.contentSources,
            socialSources: investigationInput.socialSources,
            sourceCount: investigationInput.sourceCount
        }, null, 2)
    );

    console.log(
        "INVESTIGATION_SELECTED_URLS",
        JSON.stringify(
            investigationInput.selectedSourceUrls || [],
            null,
            2
        )
    );

    // Keep an explicit machine-readable marker without any AI conclusions.
    // Groq/Loop 7 remains the reasoning layer.
    universalPackage.intelligence = {
        status: "evidence_only",
        source: "CrossEvidenceBrain",
        reasoning: null
    };
    ...
    console.log("UNIVERSAL_PACKAGE_DEBUG", {
        ...
        intelligenceStatus: "evidence_only",
        packageTextSize
    });
