...
async function PublicEvidenceHunter({
    ...
    const extractIdentity = () => {
        ...
        const brandCandidates = [
            ...
            .filter(value =>
                value &&
                value.length >= 3 &&
                !/^(www|com|org|net|online|official)$/i.test(value)
            );

        for (const brandValue of brandCandidates) {
            if (!brands.includes(brandValue)) brands.push(brandValue);
            if (brands.length >= 4) break;
        }

        const positioningDictionary = [
            "ai",
            "artificial intelligence",
            "systems",
            "system thinking",
        ...
        const normalizedCombined = normalizeIdentityText(combined);

        for (const term of positioningDictionary) {
            const normalizedTerm = normalizeIdentityText(term);
            if (
                normalizedTerm &&
                normalizedCombined.includes(normalizedTerm)
            ) {
                positioningSignals.push(term);
            }
            if (positioningSignals.length >= 6) break;
        }
        ...
        return {
            ...
            brands: [...new Set(brands.filter(Boolean))].slice(0, 4),
            positioningSignals: [...new Set(positioningSignals)].slice(0, 6),
            website,
            host,
    ...
    const brand = identity.brands?.[0] || "";
    const host = identity.host;
    const positioningSignals = Array.isArray(identity.positioningSignals)
        ? identity.positioningSignals
        : [];

    // Brand-first discovery: the public brand and its positioning are the
    // primary search entities. Avoid bare personal-name searches because
    // generic names create unrelated public-web results.
    if (brand) {
        searchQueries.push(
            q(`"${brand}"`, "brand-identity")
        );

        for (const position of positioningSignals.slice(0, 3)) {
            searchQueries.push(
                q(`"${brand}" "${position}"`, "brand-position")
            );
        }

        searchQueries.push(
            q(`"${brand}" founder OR creator OR CEO`, "brand-owner")
        );

        searchQueries.push(
            q(`"${brand}" review OR article OR interview`, "brand-mentions")
        );
    }

    // Keep the exact LinkedIn slug as a precision anchor, but do not
    // expand generic person-name searches.
    if (linkedinSlug) {
        searchQueries.push(
        ...
        if (brand) {
            searchQueries.push(
                q(`site:linkedin.com/posts "${brand}"`, "linkedin-posts-brand")
            );
        } else if (primaryName) {
            searchQueries.push(
    ...
    if (
        brand &&
        company &&
        normalizeIdentityText(brand) !== normalizeIdentityText(company)
    ) {
        searchQueries.push(
            q(`"${brand}" "${company}"`, "brand-company")
        );
    }

    if (brand && positioningSignals.length) {
        searchQueries.push(
            q(
                `"${brand}" ${positioningSignals
                    .slice(0, 3)
                    .map(value => `"${value}"`)
                    .join(" OR ")}`,
                "positioning"
    ...
    if (company) {
        searchQueries.push(
            q(`"${company}" founder OR creator`, "company")
        );
        searchQueries.push(
    ...
    if (host && brand) {
        searchQueries.push(
            q(`site:${host} "${brand}"`, "website-brand")
        );
    }
    ...
    const linkedinContentIdentityMatch = (item = {}) => {
        ...
        const brandMatch =
            (Array.isArray(identity?.brands)
                ? identity.brands
                : []
            ).some(brandName =>
                brandName &&
                text.includes(
                    normalizeIdentityText(brandName)
                )
            );

        const positioningMatch =
            (Array.isArray(identity?.positioningSignals)
                ? identity.positioningSignals
                : []
            ).some(position =>
                position &&
                text.includes(
                    normalizeIdentityText(position)
                )
            );
        ...
        if (isPost) {
            ...
            if (brandMatch && positioningMatch) return true;
        }
...
if (!identityMatched) {
    ...
    const strictHunterIdentityMatch = (candidate = {}, targetIdentity = {}) => {
        ...
        const brandNames = Array.isArray(targetIdentity?.brands)
            ? targetIdentity.brands
            : [];
        ...
        const positioningMatch =
            (Array.isArray(targetIdentity?.positioningSignals)
                ? targetIdentity.positioningSignals
                : []
            ).some(position =>
                position &&
                haystack.includes(
        ...
        if (nameMatch && (companyMatch || brandMatch)) return true;
        if (brandMatch && positioningMatch) return true;
        if (companyMatch && positioningMatch) return true;
