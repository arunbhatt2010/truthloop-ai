 ...
 * --------------------------
 * Fetches only the public social/profile URLs already discovered by the
 * supplied source. No search-engine crawling, identity expansion, ranking,
 * or external discovery is performed here.
 *
 * Goal:
 * - keep acquisition deterministic;
 * - fetch a small, controlled social set;
 * - preserve whatever public content a platform actually returns;
...
async function SocialMediaContentFetcher({
    ...
    const MAX_SOCIAL_SOURCES = MAX_GEMINI_SOCIAL_LINKS;
        ...
        [
            ...
            .filter(url => {
                ...
                return Boolean(
                    ...
                    !/(?:^|\/)(?:login|signin|signup|oauth|privacy|terms|cookies|help|support)(?:\/|$)/i.test(
    ...
    for (const url of socialUrls) {
        ...
        try {
            const response = await fetch(url, {
                ...
                headers: {
                    "User-Agent":
                        "Mozilla/5.0 (compatible; TruthLoop-SocialContentFetcher/1.0; +https://truthloop.in/)",
                    Accept:
            ...
            const page = response.ok && html.length >= 100
                ? extractSocialPage(html)
                : { title: null, description: null, visibleText: null };

            const hasContent = Boolean(
                page.visibleText || page.description || page.title
            );
            ...
            fetchedSources.push({
                ...
                fetchStatus: response.ok && hasContent ? "success" : "blocked-or-empty",
                title: page.title,
                ...
                contentLength: String(page.visibleText || "").length,
                socialLinks: [url],
                socialProfiles: [url],
                links: [],
                headings: [],
                ...
                publicEvidence: [
                    {
                        type: hasContent ? "social-page-content" : "social-profile-url",
                        sourceUrl: url,
                        value: cleanText(
                            page.visibleText ||
                            page.description ||
                ...
                evidence: [
                    {
                        type: hasContent ? "social-content" : "social-profile-url",
                        sourceUrl: url,
                        value: cleanText(
                            page.visibleText ||
                            page.description ||
            ...
            fetchedSources.push({
                ...
                evidence: [
                    {
                        type: "social-fetch-status",
                        sourceUrl: url,
                        value: error?.name === "AbortError"
                            ? "Public social source fetch timed out."
                            : "Public social source fetch failed."
                    }
                ]
            });
        } finally {
    ...
    return {
        success: socialUrls.length > 0,
        socialSources: socialUrls,
        fetchedSources,
        fetchedCount: fetchedSources.filter(
            item => item.fetchStatus === "success"
        ).length,
        attemptedCount: fetchedSources.length
    };
}
