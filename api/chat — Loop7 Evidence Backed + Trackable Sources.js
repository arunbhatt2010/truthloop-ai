...
Use the provided public evidence package and generate an evidence-backed investigation report.

Return the report in exactly these 8 section headings, each on its own line:
...
Formatting rules:
- Do NOT add emojis to the headings.
- Do NOT add markdown heading markers such as # or ##.
- Do NOT add bullet characters such as ⏩, ▶️, ➡️, -, *, or •.
- Keep each heading on its own line.
- Put the investigation content directly under its heading.

EVIDENCE RULES:
- Base every factual observation, inference, comparison, and conclusion ONLY on information present inside compressedEvidencePackage.
- Treat sourceRegistry/source records as the canonical traceability map.
- Use ONLY source IDs that actually exist in sourceRegistry.
- Every evidence-backed claim MUST include one or more citations in this exact form: [SOURCE_01].
- For multiple sources, use separate bracketed citations, e.g. [SOURCE_01] [SOURCE_07] [SOURCE_12].
- NEVER invent, renumber, merge, or modify source IDs.
- NEVER write bare SOURCE_XX when making an evidence claim; always use the bracketed form [SOURCE_XX].
- Do not cite a source unless the cited source supports the claim.
- Prefer the smallest set of directly supporting sources rather than citing every source indiscriminately.

INVESTIGATION RULES:
- Investigate patterns across the evidence; do not merely summarize the sources.
- Distinguish direct evidence from inference.
- Treat behavioral conclusions as hypotheses supported by observed public evidence, not diagnoses or certainty.
- Look for repetition, contrasts, inconsistencies, evolution over time, and relationships between sources.
- Explicitly identify meaningful contradictions when they exist.
- Do not manufacture contradictions when sources are consistent.
- State important evidence gaps when the available public evidence cannot establish a conclusion.
- Keep the report case-specific and evidence-led.

SECTION RULES:
- Investigation Summary: state the strongest evidence-backed finding and cite it.
- Behavioral Findings: list distinct recurring behavioral patterns, each grounded in citations.
- Hidden Mechanism: explain the most plausible mechanism connecting the observed behaviors, with supporting citations and clear uncertainty language where appropriate.
- Public Evidence: identify the strongest concrete observations from the public sources, with citations.
- Cross Evidence: explicitly connect evidence across different sources/platforms and cite each connection.
- Evidence Confidence: separate confidence in the observed facts from confidence in the behavioral interpretation.
- Final Reflection: synthesize the central contradiction or pattern without introducing new unsupported facts.
- One Next Action: give one evidence-consistent action that follows from the investigation; do not introduce unrelated advice.

- Ignore Loop 1-6 self reports, chat history, profile cards, user statements, private context, and truthLoopPackage.
- If evidence is insufficient, explicitly state that evidence is insufficient and cite the available sources when possible.
