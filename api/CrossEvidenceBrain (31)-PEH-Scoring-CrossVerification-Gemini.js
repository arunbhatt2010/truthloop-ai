 ...
 *
 * Design contract:
 * - Does not rename/remove any existing CEB function.
 * - Uses LinkedIn/profile URLs only as identity seeds.
 * - Searches Google, Bing and DuckDuckGo with provider fallbacks.
 * - Scores every candidate against the target identity before accepting it.
 * - Cross-verifies candidates across independent queries/providers.
 * - Produces compact, traceable evidence for downstream Gemini investigation.
 */
async function PublicEvidenceHunter({
    ...
    const extractIdentity = () => {
        ...
        const title = cleanText(mainSource?.title || "", 260);
        const description = cleanText(
            mainSource?.description || "",
            900
        );
        const visibleText = cleanText(
            mainSource?.visibleText || "",
            2600
        );
        ...
        const companyPatterns = [
            /\b(?:founder|co-founder|ceo|owner)\s+(?:of|at|@)\s+([A-Z][A-Za-z0-9&._-]{1,40}(?:\s+[A-Z][A-Za-z0-9&._-]{1,40}){0,4})/i,
            /\b(?:company|brand|product|startup)\s*[:\-]\s*([A-Z][A-Za-z0-9&._-]{1,40}(?:\s+[A-Z][A-Za-z0-9&._-]{1,40}){0,4})/i
        ];
     ...
     * -----------------------
     * Every search candidate receives an identity relevance score.
     * A candidate that does not connect to the target name/company/domain
     * is rejected instead of being allowed to pollute Loop 7.
     */
    ...
    const identitySignals = buildIdentitySignals();
    ...
    const identityScore = (item = {}) => {
        ...
        const username =
            normalizeIdentityToken(identity.linkedinUsername);
        ...
        if (username) {
            const usernameCompact = username.replace(/\s+/g, "-");
            const usernameSpaced = username;

            if (
                url.toLowerCase().includes(usernameCompact) ||
                url.toLowerCase().includes(username.replace(/\s+/g, "")) ||
                text.includes(usernameSpaced)
            ) {
                score += 50;
            }
        }
    ...
    /*
     * Cross Evidence Verification Engine
     * -----------------------------------
     * Evidence gains trust when independent searches/providers converge on
     * the same target. Verification is deterministic and traceable.
     */
    const buildEvidenceKey = (item = {}) => {
        const url = normalizeSearchUrl(item.url || "");
    ...
    if (linkedinSlug) {
        ...
        searchQueries.push(
            q(
                `site:linkedin.com/in "${linkedinSlug}"`,
                "linkedin-profile"
            )
        );
        ...
        if (primaryName) {
            searchQueries.push(
                q(
                    `site:linkedin.com/posts "${primaryName}"`,
                    "linkedin-posts"
                )
            );
    ...
    if (primaryName) {
        searchQueries.push(
            q(
                `"${primaryName}" interview OR podcast OR article`,
                "public-mentions"
            )
        );
        ...
        searchQueries.push(
            q(
                `"${primaryName}" YouTube OR Medium OR Substack OR Reddit`,
                "platform-expansion"
            )
        );
    }
    ...
    if (company) {
        ...
        searchQueries.push(
            q(
                `"${company}" blog OR article OR interview`,
                "company-content"
            )
        );
    }
    ...
    for (const item of searchQueries) {
        ...
        if (!item.query || seenQueries.has(key)) {
            continue;
        }
        ...
        if (dedupedQueries.length >= MAX_QUERIES) {
            break;
        }
    }
    ...
    const headers = {
        "User-Agent":
            "Mozilla/5.0 (compatible; TruthLoop-PublicEvidenceHunter/2.0; +https://truthloop.in/)",
        "Accept":
    ...
    const addResult = (item = {}) => {
        ...
        if (
            /^(?:accounts|support|help|policies|privacy|terms)\./i
                .test(hostName)
        ) {
            return;
        }
         ...
         * allow the target domain and exact social identity at a lower
         * threshold, but do not accept generic web noise.
         */
        const directLinkedInIdentity =
            detectPlatform(candidate.url) === "linkedin" &&
            identity.linkedinUsername &&
            candidate.url
                .toLowerCase()
                .includes(
        ...
        if (
            candidate.identityScore < 20 &&
            !directLinkedInIdentity &&
            !targetDomain
        ) {
            return;
        }
    ...
    /*
     * Evidence scoring + cross verification
     */
    ...
    const scoreResult = (item) => {
        ...
        if (
            path.match(
                /\/(blog|article|articles|post|posts|news|insight|story|writing|research|podcast|interview|video|case-study|case-studies)/i
            )
        ) {
            score += 20;
        }
        ...
        if (
            /\/(privacy|terms|cookie|login|signin|signup|support|help|contact|search)(\/|$)/i
                .test(path)
        ) {
            score -= 50;
        }
    ...
    const crossEvidenceMap =
        new Map();

    for (const item of results) {
        const key =
            buildEvidenceKey(item);

        if (!key) continue;

        const entry =
            crossEvidenceMap.get(key) || {
                queries: new Set(),
                providers: new Set(),
                identityScores: [],
                items: []
    ...
    for (const item of results) {
        ...
        const providerSupport =
            cross?.providers?.size ||
            0;

        const baseScore =
            scoreResult(item);

        const directIdentityScore =
            Number(
                item.identityScore ||
                0
            );
         ...
         * - exact domain/profile evidence can survive with one source;
         * - everything else needs a meaningful identity score.
         */
        const directTarget =
            (
                identity.host &&
            ...
            .map(
                (item, index) => ({
                    ...
                    identityScore:
                        item.identityScore,
                    crossQueryCount:
                        item.crossQueryCount,
                    crossProviderCount:
                        item.crossProviderCount,
                    verificationScore:
                        item.verificationScore
                })
            );
    ...
    const verificationSummary = {
        ...
        multiQueryEvidence:
            ranked.filter(
                item =>
                    item.crossQueryCount >= 2
            ).length,
        multiProviderEvidence:
            ranked.filter(
                item =>
                    item.crossProviderCount >= 2
            ).length
    };
    ...
    return {
        ...
        reason:
            ranked.length
                ? null
                : "No identity-verified public search results returned."
    };
}
...
export async function loadCrossEvidenceBrain({
    ...
    geminiInput.publicEvidenceHunter = {
        ...
        verification: publicEvidenceHunter?.verification || {},
