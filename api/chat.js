...
const loop7SignalMaster =
  compressedEvidencePackage?.loop7Package?.evidenceUniverse?.signalMaster ||
  {};

const loop7EvidenceCoverage =
  compressedEvidencePackage?.loop7Package?.evidenceCoverage ||
  {};

loop7AiUserPayload = {
  caseSpine: loop7CaseForAI,
  evidenceRegistry: loop7EvidenceForAI,
  signalMaster: loop7SignalMaster,
  evidenceCoverage: loop7EvidenceCoverage
};

console.log(
  "LOOP7_CANONICAL_EVIDENCE_PAYLOAD",
  JSON.stringify({
    ...
    deepSignalFamilyCount:
      loop7SignalMaster?.deepSignalFamilyCount || 0,
    deepSignalCount:
      loop7SignalMaster?.deepSignalCount || 0
  })
);
