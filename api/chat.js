...
OUTPUT BUDGET — TARGET ~2400 TOKENS TOTAL
Aim approximately for: 200 / 300 / 400 / 300 / 400 / 300 / 400 / 100 tokens for the eight sections respectively.
These are quality targets, not excuses to omit evidence. Be dense, specific, and investigative.
...
if (loopLevel === 7) {
  ...
  const reportPassesInvestigationGate = value => {
    ...
    return (
      ...
      mustContain("📊 Evidence Confidence", ["confidence assessment"], 180) &&
    ...
    try {
            ...
            messages: [
              {
                ...
                content:
                  ...
                  "The previous draft failed the strict completeness/investigation gate. Rewrite from the evidenceRegistry with PUBLIC EVIDENCE as the primary authority (~80%) and Loop 1–6 as supporting context (~20%). " +
                  "Every section must contain substantive investigation, not summary, praise, repetition, or unsupported psychology. " +
                  "Actively inspect dates, recency, publishing gaps, behavioral changes, contradictions, and evidence gaps when supported. " +
                  "If a section contains lipa-poti or merely repeats the user's narrative, rewrite that section before returning. " +
                  "Do not turn missing evidence into proof of absence, motive, failure, or intent. " +
                  "Finish all eight sections completely within the 2400-token budget."
              },
              {
                role: "user",
      ...
      if (retryResponse.ok) {
        ...
        if (reportPassesInvestigationGate(retryReply)) {
          ...
          console.error(
            "LOOP7_REPORT_FINAL_GATE_FAILED",
            "Single retry exhausted; no additional Groq retry will be attempted."
          );
        }
      } else {
