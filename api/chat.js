...
/* ==========================================
   LOOP 7 TRACKABLE SOURCE INDEX
   Deterministic / no AI call.
========================================== */
function buildLoop7EvidenceSourceIndex(value = {}) {
  const entries = [];
  const seen = new Set();
  ...
  const walk = (node, path = "root") => {
    ...
    let url = "";
    for (const key of ["url","sourceUrl","canonicalUrl","profileUrl","postUrl","articleUrl","link"]) {
      if (node?.[key]) { url = normalize(node[key]); if (url) break; }
    }
    if (url && !seen.has(url)) {
      seen.add(url);
      const label = clean(
        node?.sourceName || node?.sourceTitle || node?.sourcePlatform ||
        node?.platform || node?.source || path.split('.').slice(-1)[0] || "Evidence source"
      ).slice(0, 100);
      entries.push({
        sourceId: `SOURCE_${String(entries.length + 1).padStart(2, "0")}`,
        label,
        url
      });
    }
...
CASE INPUT
The TruthLoop package contains compressed profile-card evidence from Loops 1–6.
The evidence package contains verified public evidence and trackable URLs.
The source index lists every discovered trackable public source.

INVESTIGATION FOCUS
Investigate the PERSON and the recurring pattern revealed by the completed Loop 1–6 case.
Do not review the brand, website, audience size, follower count, connections, or market traction unless directly necessary to explain the person's investigated pattern. Do not mention follower count or connections in the final report.

EVIDENCE ORDER
1. Loop 1–6 profile-card evidence = internal case spine.
2. Verified public evidence = external corroboration/challenge/context.
3. Cross-source relationships = convergence, tension, contradiction, or evidence gap.

TRACKABLE SOURCE USAGE — MANDATORY
The source index is evidence, not decoration.
Use the relevant source IDs and exact URLs throughout the report.
When 8 or more source IDs are available, use at least 8 DISTINCT source IDs across the report.
If fewer than 8 exist, use every relevant source that genuinely supports a finding.
Do not repeatedly cite only the first website and one LinkedIn profile when other relevant sources exist.
Never cite a source that does not support the claim.
Never invent, shorten, or modify a URL.

SECTION SOURCE DISTRIBUTION
📋 Investigation Summary: use at least 2 distinct supporting sources when available.
🧩 Behavioral Findings: use at least 2 distinct sources when available.
⚙ Hidden Mechanism: use at least 2 independent observations/sources when available.
🌐 Public Evidence: use at least 3 distinct public sources when available.
🔍 Cross Evidence: compare at least 3 distinct relevant sources when 3+ exist.
📊 Evidence Confidence: name the strongest and weakest evidence sources when available.
💡 Final Reflection: ground the conclusion in converging evidence.
🎯 One Next Action: tie it to the strongest verified constraint or evidence gap.
...
Never manufacture an aha moment or contradiction.
...
Do not add a preface, appendix, or extra headings.
Do not use the label "Reasoning".

QUALITY
The report must explain what the person repeatedly does, avoids, protects, delays, or rationalizes; what evidence supports it; what the public evidence confirms or challenges; and what the user is most likely not seeing.
Do not let public brand language replace the Loop 1–6 case.
Each section must add new investigative value.
Prefer fewer strong findings over many weak observations.
A short evidence-backed section is better than fabricated detail.
...
✓ Relevant source diversity used
✓ Trackable URLs preserved
✓ Loop 1–6 case evidence used
✓ Public evidence used to corroborate/challenge the case
...
If evidence is genuinely unavailable, write:
