...
if (loopLevel === 7) {
  ...
  const reportIsComplete = value => {
    ...
    return requiredLoop7Sections.every(heading => {
      const body = getSectionBody(normalized, heading);
      return body.length >= 24 && !emptyOnly.has(body.toLowerCase());
    });
  };
  ...
  if (!reportIsComplete(reply)) {
    console.log("LOOP7_REPORT_RETRY", "incomplete_or_truncated");
    ...
    try {
            ...
            messages: [
              {
                ...
                content:
                  ...
                  "The previous draft was incomplete. Return the same eight-section format again. " +
                  "Every section must contain substantive text. Finish all eight sections. " +
      ...
      if (retryResponse.ok) {
        ...
        if (reportIsComplete(retryReply)) {
