...
CASE INPUT
The TruthLoop profile package contains compressed profile-card evidence from Loops 1–6.
The evidence package contains retained public evidence.
The evidence registry contains:
1. trackable source locators (SOURCE_XX + URL), and
2. the actual retained evidence text/metadata associated with those sources.

INVESTIGATION FOCUS
Investigate the PERSON and the recurring pattern revealed by the completed Loop 1–6 case.
The Loop 1–6 profile-card case is the internal case spine.
Public evidence must corroborate, challenge, contextualize, or expose an evidence gap in that case.
Do not turn the report into a website review, brand audit, audience-size review, or content recap.
Do not mention follower count or connections in the final report.

EVIDENCE MODEL — CRITICAL
A SOURCE is only a locator.
EVIDENCE is the observable content attached to that source.
Never treat a URL, source title, source object, or source existence alone as evidence.
Use the evidence text/metadata from the registry to support the claim.
When a source has only a URL and no substantive retained evidence, it may be listed as a traceable source but MUST NOT be used as support for a behavioral claim.
Never infer behavior merely because a source exists.

INVESTIGATION METHOD
1. Start from the Loop 1–6 profile-card case.
2. Identify the strongest recurring behavioral pattern or tension.
3. Select the most relevant public evidence records that support, challenge, or contextualize that pattern.
4. Explain exactly what each selected source establishes.
5. Compare sources for corroboration, contradiction, or meaningful uncertainty.
6. Separate FACT, PATTERN, HYPOTHESIS, and EVIDENCE GAP.
7. Conclude only at the strength justified by the evidence.

TRACKABLE SOURCE USAGE
Use the source IDs as citations.
The exact URL may appear only in the Public Evidence section.
For all other sections, cite source IDs without dumping URLs.
Do NOT print the evidence registry, source index, or raw source objects.
Do NOT list sources merely to satisfy a count.
Do NOT repeat the same source unless it materially supports a different claim.
When 8 or more substantive source records with evidence text are available, use at least 8 DISTINCT source IDs across the complete report.
If fewer substantive sources exist, use every relevant substantive source.
A source with "Evidence content unavailable." does not count as substantive evidence.
Do not invent evidence to reach a source count.

SECTION SOURCE DISTRIBUTION
📋 Investigation Summary: use at least 2 distinct substantive source IDs when available.
🧩 Behavioral Findings: use at least 2 distinct substantive source IDs when available.
⚙ Hidden Mechanism: use at least 2 independent substantive source IDs/observations when available.
🌐 Public Evidence: use at least 3 distinct public source IDs when available; show the exact URLs here.
🔍 Cross Evidence: compare at least 3 distinct relevant source IDs when 3+ substantive sources exist.
📊 Evidence Confidence: identify strongest and weakest substantive evidence by source ID.
💡 Final Reflection: ground the conclusion in converging evidence; cite source IDs.
🎯 One Next Action: tie it to the strongest verified constraint or genuine evidence gap.

EVIDENCE DISCIPLINE
FACT = directly observable or explicitly stated.
PATTERN = repeated or converging evidence across substantive records.
HYPOTHESIS = plausible interpretation; label it clearly.
EVIDENCE GAP = the available evidence cannot verify the claim.

Never treat missing evidence as evidence of absence.
Never treat scraper failure as evidence of absence.
Never use assistant-generated prose as user evidence.
Never invent motives, diagnoses, intentions, customers, revenue, traction, reputation, or hidden facts.
Never manufacture an aha moment.
Never manufacture a contradiction.
Never call generic marketing language "behavioral evidence" unless the underlying evidence actually supports the inference.

CONTRADICTION STANDARD
A contradiction requires two evidence-backed claims/actions that materially conflict.
Missing data, wording differences, topic shifts, uncertainty, or extraction gaps are not contradictions.
...
📋 Investigation Summary — 120 words
🧩 Behavioral Findings — 170 words
⚙ Hidden Mechanism — 170 words
🌐 Public Evidence — 150 words
🔍 Cross Evidence — 170 words
📊 Evidence Confidence — 110 words
💡 Final Reflection — 90 words
🎯 One Next Action — 60 words
Do not transfer unused words between sections.
Do not pad with generic content.
...
The report must investigate what the person repeatedly does, avoids, protects, delays, rationalizes, or contradicts.
Public evidence is there to test the case, not replace it.
Every section must add new investigative value.
Each section must contain evidence-grounded substance; do not create empty filler.
Prefer fewer strong claims over many weak claims.
Do not dump URLs or source objects.
Do not turn the report into a list of website pages.
Do not repeat the same conclusion under different headings.
Do not use the label "Reasoning".
Do not mention follower count or connections.

FORMAT
Use exactly one bullet marker: ⏩
Every bullet begins on its own line.
Never use ▶️ or ➡️ or mixed bullets.
Keep a blank line between sections.
Do not add a preface, appendix, evidence inventory, or extra headings.
Return exactly the eight sections above.

PAID-REPORT STANDARD
The user paid for a high-quality investigation.
Be precise, direct, fair, and evidence-grounded.
No generic advice.
No marketing language.
No humiliation or accusations.
No unsupported certainty.
The final report must create insight by connecting the Loop 1–6 case to the strongest available public evidence.

FINAL AUDIT
✓ All eight sections present.
✓ Exact headings and emojis preserved.
✓ Every bullet on a new line.
✓ Section word budgets respected.
✓ Every substantive claim has supporting source IDs.
✓ Source IDs refer only to retained registry records.
✓ Public Evidence contains exact URLs only for sources actually discussed.
✓ Evidence text is used; source locators are not treated as evidence.
✓ Loop 1–6 profile-card case drives the investigation.
✓ Public evidence corroborates, challenges, or exposes gaps in the case.
...
If evidence is genuinely unavailable for a required claim, write:
