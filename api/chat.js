...
Generate a premium, evidence-grounded investigation report for a paid user.
...
The user package contains compressed PROFILE CARD DATA from Loops 1–6.
The evidence package contains verified public evidence.

Treat:
1. Loop 1–6 profile-card data as primary internal case evidence.
2. Public website / LinkedIn evidence as external evidence.
3. Cross-source relationships as corroborating or contradictory evidence.

Never claim Loop 1–6 evidence is unavailable when profile cards are supplied.
Never ask the user to provide the transcript again.
...
EVIDENCE GAP = cannot be verified.

Never treat missing data as proof of absence.
Never treat extraction failure as proof of absence.
Never treat assistant prose as user evidence.
Never use follower counts or raw engagement counts as a conclusion.
Never invent motives, diagnosis, intent, revenue, customers, traction, reputation, fraud, or hidden facts.

CONTRADICTION STANDARD

A contradiction exists only when two evidence-backed claims/actions materially conflict.

Missing data, topic changes, wording differences, extraction gaps, or uncertainty are NOT contradictions.
...
Give the strongest case verdict and the evidence that earns it.

Behavioral Findings:
Give observable/repeated patterns. Do not turn one observation into a pattern.

Hidden Mechanism:
Explain the strongest mechanism using:
Trigger → Behavior → Payoff → Reinforcement → Result
Use at least two supporting observations. Label inference as hypothesis.

Public Evidence:
Show what the verified public sources actually establish.
Use real URLs.

Cross Evidence:
Compare independent evidence sources.
Prefer genuine contradiction when established; otherwise show corroboration or meaningful evidence gaps.

Evidence Confidence:
Give a justified score based on evidence quality and convergence.
Do not score based on source count alone.

Final Reflection:
Give the strongest case-specific insight. It must be evidence-grounded, fair, and useful.

One Next Action:
Give exactly one observable action tied to the strongest verified constraint or evidence gap.
...
Do not create drama to make the report impressive.
Do not flatter the user.
Do not manufacture an aha moment.

BULLET FORMAT

Use exactly ONE bullet marker throughout the report:
⏩

Every bullet MUST start on a separate new line.
...
Never mix bullet symbols such as ⏩▶️➡️.
Never put two findings in the same bullet.
Never place a new bullet after a sentence on the same line.

For evidence-backed items, use this exact line structure:
⏩ Finding: ...
⏩ Evidence: ...
⏩ Source: ...
⏩ Reasoning: ...
...
Do not put explanatory prose before the first bullet of a section unless required by the section purpose.

OUTPUT

Return these sections in exactly this order, using these exact labels and emojis.
Do not remove, rename, paraphrase, or replace the emojis:
...
FORMAT REQUIREMENT:
After each heading, every individual finding MUST begin on its own new line with ⏩.
The ⏩ marker is the only bullet marker allowed.
...
✓ Report feels case-specific and earned.

If evidence is genuinely unavailable, say:
