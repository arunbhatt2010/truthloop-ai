...
async function loadEvidenceCompressionBrain({
  ...
  /*
   * HARD COMPRESSION ONLY
   * ---------------------
   * ECB may shorten text, but must NOT:
   * - select evidence
   * - rank evidence
   * - remove sources
   * - remove evidence items
   * - deduplicate evidence
   * - slice arrays during compression
   *
   * The structure remains intact. Only string values are shortened.
   */
  ...
  function compressStringTree(value, maxChars, key = "") {
    if (typeof value === "string") {
      if (isProtectedStringKey(key)) {
        return value;
      }
    ...
    if (Array.isArray(value)) {
      /*
       * IMPORTANT:
       * Preserve every array element.
       * No slice/filter is performed here.
       */
      return value.map(item =>
        compressStringTree(item, maxChars, key)
      );
    }
  ...
  /*
   * Progressive text-only passes.
   * We never delete an evidence item/source.
   */
  const ECB_TEXT_COMPRESSION_PASSES = [
    900,
    600,
    400,
    250,
  ...
  for (const maxTextChars of ECB_TEXT_COMPRESSION_PASSES) {
    ...
    /*
     * Keep the exact package structure.
     * Only string values may have changed.
     */
    for (const [key, value] of Object.entries(compressedPackage)) {
   ...
   * ABSOLUTE HARD LOCK
   * ------------------
   * Never export > 10,000 characters.
   * If text-only compression cannot achieve the limit,
