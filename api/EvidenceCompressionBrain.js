...
async function loadEvidenceCompressionBrain({
  ...
  if (finalSize > MAX_TOTAL_PACKAGE_CHARS) {
    ...
    const emergencyLevels = [120, 80, 50, 30, 15, 5];
    ...
    if (finalSize > MAX_TOTAL_PACKAGE_CHARS) {
      /*
       * Last-resort compact representation.
       * Source URLs/IDs/dates remain available through the registry.
       * This branch should only be reachable when upstream metadata
      ...
      finalPackage = {
        ...
        evidenceUniverse: {
          ...
          sourceRegistry: sourceRegistry.map(source => ({
            sourceId: source.sourceId,
            sourceType: source.sourceType,
            title: cleanText(source.title).slice(0, 120) || null,
            url: source.url || null,
            date: source.date || null
          })),
          websiteSources: [],
          linkedinProfile: profile
            ? {
                ...
                name: cleanText(profile.name).slice(0, 120) || null,
                headline: cleanText(profile.headline).slice(0, 120) || null,
                about: cleanText(profile.about).slice(0, 120) || null
              }
            : null,
          linkedinPosts: linkedinPosts.map(post => ({
            ...
            url: post.url || null,
            date: post.date || null,
            title: cleanText(post.title).slice(0, 120) || null,
            content: cleanText(post.content).slice(0, 120) || null
          })),
          linkedinArticles: linkedinArticles.map(article => ({
            ...
            url: article.url || null,
            date: article.date || null,
            title: cleanText(article.title).slice(0, 120) || null,
            content: cleanText(article.content).slice(0, 120) || null
          })),
          githubEvidence: githubSource
            ? trimNode(githubSource, 60)
            : null,
          signalMaster: {}
        },
        evidenceCoverage: {
